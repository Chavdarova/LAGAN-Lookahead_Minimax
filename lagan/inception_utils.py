# -*- coding: utf-8 -*-
"""
    The code in this script is adapted from `https://github.com/ajbrock/BigGAN-PyTorch`,
    and it is used for computing FID/IS scores in PyTorch.
    It's advantage relative to the original FID and IS implementations (both in tensorflow),
    is that it runs notably faster (approximately five times faster).
    IMPORTANT NOTE: although this script is useful to more frequently monitor the training, its
    numerical values in practice differ from those obtained with the original tf implementations.
    As the latter are used by the majority of the papers, we recommend using the original tf
    implementations for IS and FID for official benchmarks and publication.
...
def prepare_z_(G_batch_size, dim_z, device='cuda', z_var=1.0):
